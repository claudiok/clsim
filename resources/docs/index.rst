...
.. copyright  (C) 2011
.. Claudio Kopper for
.. the KM3NeT Consortium / the IceCube Collaboration
.. 
.. $Id$
.. 
.. @version $Revision$
.. @date $LastChangedDate$
.. @author Claudio Kopper <ckopper@icecube.wisc.edu> $LastChangedBy$
...
also set the ``MMCTrackListName`` to ``None``. This will apply *Geant4* to all
