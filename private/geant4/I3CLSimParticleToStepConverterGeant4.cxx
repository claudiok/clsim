...
I3CLSimParticleToStepConverterGeant4::I3CLSimParticleToStepConverterGeant4(uint32_t maxQueueItems)
:
...
electronPositronMinEnergyForSecondary_(std::numeric_limits<double>::infinity()),
electronPositronMaxEnergyForSecondary_(0.),
initialized_(false),
...
void I3CLSimParticleToStepConverterGeant4::SetElectronPositronMinEnergyForSecondary(double val)
{
    if (initialized_)
        throw I3CLSimParticleToStepConverter_exception("I3CLSimParticleToStepConverterGeant4 already initialized!");

    if (val<0.)
        throw I3CLSimParticleToStepConverter_exception("Negatve energies are invalid!");
    
    electronPositronMinEnergyForSecondary_=val;
}

void I3CLSimParticleToStepConverterGeant4::SetElectronPositronMaxEnergyForSecondary(double val)
{
    if (initialized_)
        throw I3CLSimParticleToStepConverter_exception("I3CLSimParticleToStepConverterGeant4 already initialized!");
    ...
    electronPositronMaxEnergyForSecondary_=val;
}

double I3CLSimParticleToStepConverterGeant4::GetElectronPositronMinEnergyForSecondary() const
{
    return electronPositronMinEnergyForSecondary_;
}

double I3CLSimParticleToStepConverterGeant4::GetElectronPositronMaxEnergyForSecondary() const
{
    return electronPositronMaxEnergyForSecondary_;
}
...
void I3CLSimParticleToStepConverterGeant4::Geant4Thread_impl(boost::this_thread::disable_interruption &di)
{
    ...
    const std::string DATA_BASEDIR = "share/geant4-4.9.2.p01/data";
    ...
    if (!dummy) {
        ...
        setenv("G4LEDATA", (I3_PORTS + "/" + DATA_BASEDIR + "/G4EMLOW6.2").c_str(), 1);
        setenv("G4NEUTRONHPDATA", (I3_PORTS + "/" + DATA_BASEDIR + "/G4NDL3.13").c_str(), 1);
        setenv("G4ABLADATA", (I3_PORTS + "/" + DATA_BASEDIR + "/G4ABLA3.0").c_str(), 1);
    }
    
    const double maxBetaChangePerStep = 10.*perCent;
    const double maxNumPhotonsPerStep = 200.;
    
    // this thing stores all the steps generated by Geant4, sorted by the number
    // of Cherenkov photons they generate
    I3CLSimStepStorePtr stepStore(new I3CLSimStepStore( (isnan(maxNumPhotonsPerStep)||(maxNumPhotonsPerStep<0.))?0:(static_cast<uint32_t>(maxNumPhotonsPerStep*1.5)) ));
    ...
    G4VModularPhysicsList *physics = factory->GetReferencePhysList("QGSP_BERT_EMV");
    delete factory;
    ...
    TrkEventAction *theEventAction = new TrkEventAction(maxBunchSize_,
                                                        stepStore,
                                                        (electronPositronMinEnergyForSecondary_/I3Units::GeV)*GeV,
                                                        (electronPositronMaxEnergyForSecondary_/I3Units::GeV)*GeV,
                                                        queueFromGeant4_,
                                                        di,
                                                        maxRefractiveIndex);
    runManager->SetUserAction(theEventAction);      // runManager now owns this pointer
    //runManager->SetUserAction(new TrkSteppingAction()); // runManager now owns this pointer
    ...
    //UI->ApplyCommand("/random/setSeeds 574860 0");
    CLHEP::HepRandom::setTheSeed(574860); // value [0,900000000]
    ...
    for (;;)
    {
        ...
        // 
        // // we finished simulating a particle.
        // // signal the caller by putting it's id on the 
        // // output queue.
        // {
        //     boost::this_thread::restore_interruption ri(di);
        //     try {
        //         queueFromGeant4_->Put(particleIdentifier);
        //     } catch(boost::thread_interrupted &i) {
        //         log_warn("G4 thread was interrupted. closing.");
        //         break;
        //     }
        // }
        // 
    ...
    //delete theUserHookForAbortState;
